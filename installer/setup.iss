...
Source: "..\src\healthcheck.py";          DestDir: "{app}";              Flags: ignoreversion
...
[Messages]
FinishedLabel=Установка [name] завершена. Приложение можно запустить через ярлыки в меню «Пуск».%n%nДля работы 1C MCP Bridge откройте новый разговор ChatGPT Desktop в режиме Work или Codex. Обычный Chat локальный MCP не использует.%nПроверка: /mcp → 1c-bridge → Включено → спросите показать список баз.
