...
* **Пароли 1С** хранятся в `%PROGRAMDATA%\1cMcpBridge\databases.json`. Файл доступен только текущему пользователю. По возможности используйте **аутентификацию Windows** вместо логина/пароля.
* **HTTP-сервер** (для Qwen) слушает только `127.0.0.1` — доступ из сети закрыт. Без флага `--insecure-host` привязаться к внешнему интерфейсу нельзя.
...
* Для продуктивных баз рекомендуется **отдельный пользователь 1С с правами только на чтение**.
...
* **v0.4.0** *(текущая)* — мульти-клиент (Claude, Qwen, Kimi, Reasonix), HTTP-сервер, импорт JSON.
* **v0.5.0** *(планируется)* — DPAPI для паролей, тайм-ауты COM, юнит-тесты, подпись установщика
