...
1. Запустите ярлык **«1C Bridge (HTTP-сервер для Qwen)»** из меню Пуск.
   Сервер запустится в фоне (без окна). Остановить — Диспетчер задач → `pythonw.exe`.
...
* **v0.3.1** *(текущая)* — мульти-клиент (Claude, Qwen, Kimi, Reasonix), импорт JSON, авто-конфигурация.
